...
title Audion Office OCR AI - Cleanup
...
if not exist "%BASE_DIR%\system_core\ui_nicegui\app.py" (
  ...
  if not "%NO_PAUSE%"=="1" call :WAIT_KEY
  exit /b 1
)
...
echo ======================================================================
echo   AUDION OFFICE OCR AI - LEAN SOURCE CLEANUP
echo ======================================================================
echo Root:
echo   %BASE_DIR%
echo.
...
echo   - input, output, logs, report, workspace, data, cache, release contents
echo   - runtime, wheelhouse, tools, install\download contents
echo   - optional OCR/preprocess payloads under tools\ (Real-ESRGAN, llama.cpp, Surya)
...
echo.
echo Protected: system_core source, config, docs, launchers, install scripts, install\tools helpers, release policies.
echo.
...
echo [OK] runtime, wheelhouse, tools, cache, install\download and generated work folders were recreated empty.
goto cleanup_done
...
:cleanup_done
if not "%AUTO_YES%"=="1" if not "%NO_PAUSE%"=="1" call :WAIT_KEY
exit /b 0
...
echo   /NOPAUSE, --no-pause  Do not wait for a key before exit.
exit /b 0
:RemoveNamedDirs
set "DIR_NAME=%~1"
