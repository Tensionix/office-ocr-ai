...
title Audion Office OCR AI - Project Launcher
...
cls
echo ======================================================================
echo   AUDION OFFICE OCR AI - PROJECT LAUNCHER
echo ======================================================================
echo Root:   %BASE_DIR%
...
:FZF_MENU
> "%MENU_FILE%" echo [01] OFFICE TEXT WITHOUT OCR                 ^| extract_fast             ^| GUI service: selectable text to Markdown
>>"%MENU_FILE%" echo [02] LOCAL OCR - TESSERACT                   ^| ocr_tesseract            ^| OCR Brick to DocumentModel + DOCX
>>"%MENU_FILE%" echo [03] LOCAL OCR - SURYA                       ^| ocr_surya                ^| OCR Brick layout engine + DOCX
...
>>"%MENU_FILE%" echo [09] WORKBENCH LOCAL REVIEW                  ^| workbench_review         ^| local review artifacts
>>"%MENU_FILE%" echo [10] WORKBENCH OPENAI RESOLVER               ^| workbench_openai         ^| crop-only repair resolver
>>"%MENU_FILE%" echo [11] WORKBENCH GEMINI RESOLVER               ^| workbench_gemini         ^| crop-only repair resolver
>>"%MENU_FILE%" echo [12] WORKBENCH COORDINATES                   ^| workbench_coordinates    ^| coordinate table export
>>"%MENU_FILE%" echo [13] MISTRAL + YANDEX + TESSERACT            ^| ocr_mistral_fusion       ^| verified fusion + DOCX/report
>>"%MENU_FILE%" echo.
>>"%MENU_FILE%" echo [14] BUILD DOCX                              ^| build_docx               ^| compile Markdown to DOCX
>>"%MENU_FILE%" echo [15] BUILD DOCX FROM LLM MARKDOWN            ^| build_docx_llm           ^| dense layout for LLM Markdown
>>"%MENU_FILE%" echo [16] BUILD PDF                               ^| build_pdf                ^| DOCX to PDF through Word COM
>>"%MENU_FILE%" echo [17] DEV MARKDOWN PDF                        ^| build_dev_pdf            ^| Chromium light/dark PDF from input
>>"%MENU_FILE%" echo [18] BUILD PPTX                              ^| build_pptx               ^| Markdown to presentations
>>"%MENU_FILE%" echo [19] BUILD XLSX                              ^| build_xlsx               ^| Markdown tables to Excel
>>"%MENU_FILE%" echo.
>>"%MENU_FILE%" echo [20] PROJECT STATUS                          ^| run_main                 ^| project folders and runtime
>>"%MENU_FILE%" echo [21] ENV DOCTOR                              ^| run_doctor               ^| runtime and import checks
>>"%MENU_FILE%" echo [22] CHECK OPENAI MODELS                     ^| check_models_openai      ^| query models for current API key
>>"%MENU_FILE%" echo [23] CHECK GEMINI MODELS                     ^| check_models_gemini      ^| query models for current API key
>>"%MENU_FILE%" echo.
>>"%MENU_FILE%" echo [24] OPEN input                              ^| open_input               ^| sources and Markdown
>>"%MENU_FILE%" echo [25] OPEN PROJECT ROOT                       ^| open_root                ^| project workspace
>>"%MENU_FILE%" echo [26] OPEN output                             ^| open_output              ^| all results
>>"%MENU_FILE%" echo [27] OPEN logs                               ^| open_logs                ^| logs
>>"%MENU_FILE%" echo [28] COPY output TO input                    ^| copy_output_md           ^| return generated files to input
>>"%MENU_FILE%" echo.
>>"%MENU_FILE%" echo [29] TOOLS LAUNCHER                          ^| tools                    ^| utility tools
>>"%MENU_FILE%" echo.
>>"%MENU_FILE%" echo [00] EXIT                                    ^| exit                    ^| close

type "%MENU_FILE%" | "%FZF_EXE%" --prompt="audion@office-ocr-ai [PROJECT] > " --pointer=">" --header="Pick step:" --layout=reverse --border="rounded" --info=hidden --margin=1,2 > "%RES_FILE%"
...
:FALLBACK_MENU
echo [1] Office text without OCR
echo [2] Local OCR - Tesseract
echo [3] Local OCR - Surya
...
echo [9] Workbench local review
echo [A] Workbench OpenAI resolver
echo [B] Workbench Gemini resolver
echo [C] Workbench coordinates
echo [D] Mistral + Yandex + Tesseract
echo.
echo [E] Build DOCX
echo [F] Build DOCX from LLM Markdown
echo [G] Build PDF
echo [H] DEV Markdown PDF
echo [I] Build PPTX
echo [J] Build XLSX
echo.
echo [K] Project status
echo [L] Env doctor
echo [M] Check OpenAI models
echo [N] Check Gemini models
echo.
echo [O] Open input
echo [P] Open project root
echo [Q] Open output
echo [R] Open logs
echo [S] Copy output to input
echo.
echo [T] Tools launcher
echo.
echo [0] Exit
echo.
choice /C 123456789ABCDEFGHIJKLMNOPQRST0 /N /M "Select: "
...
if not exist "%BASE_DIR%\output\" (
  echo [WARN] Folder not found: "%BASE_DIR%\output"
  if not defined AUDION_NO_PAUSE pause
  goto MAIN
)
...
if %RC% GEQ 8 (
  ...
  echo [OK] Markdown copied from output to input
)
if not defined AUDION_NO_PAUSE pause
goto MAIN
