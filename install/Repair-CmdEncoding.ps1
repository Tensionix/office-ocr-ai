...
function Get-CmdEncodingState {
    ...
    [PSCustomObject]@{
        ...
        ValidUtf8 = $validUtf8
        ErrorText = $errorText
        Ok = (-not $hasBom -and $loneLf -eq 0 -and $loneCr -eq 0 -and $validUtf8)
    }
}
...
if (-not $Quiet) {
    foreach ($state in $fixedStates) {
        Write-Host ("[FIXED] {0} BOM={1} CRLF={2} LoneLF={3} LoneCR={4} UTF8={5}" -f $state.RelativePath, $state.HasBom, $state.Crlf, $state.LoneLf, $state.LoneCr, $state.ValidUtf8)
    }
}
...
if ($badStates.Count -gt 0) {
    ...
    foreach ($state in $badStates) {
        Write-Host ("  - {0} BOM={1} CRLF={2} LoneLF={3} LoneCR={4} UTF8={5} {6}" -f $state.RelativePath, $state.HasBom, $state.Crlf, $state.LoneLf, $state.LoneCr, $state.ValidUtf8, $state.ErrorText)
    }
    exit 1
}
