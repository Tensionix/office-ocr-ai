...
title Audion Office OCR AI - Make Release Archive

set "SCRIPT_DIR=%~dp0"
if "%SCRIPT_DIR:~-1%"=="\" set "SCRIPT_DIR=%SCRIPT_DIR:~0,-1%"
for %%A in ("%SCRIPT_DIR%\..") do set "ROOT=%%~fA"
set "PROJECT_NAME=Audion Office OCR AI"
...
robocopy "%ROOT%" "%STAGE_PROJECT%" /E /R:1 /W:1 /NFL /NDL /NJH /NJS /NP /XD "%ROOT%\.git" "%ROOT%\release" "%ROOT%\.venv" "%ROOT%\venv" "%ROOT%\__pycache__" "%ROOT%\.pytest_cache" "%ROOT%\config\keys" "%ROOT%\input" "%ROOT%\output" "%ROOT%\logs" "%ROOT%\report" "%ROOT%\workspace" "%ROOT%\data" "%ROOT%\cache" "%ROOT%\tmp" "%ROOT%\PDF" "%ROOT%\._runtime" "%ROOT%\install\download" "%ROOT%\work\build" /XF "api_key*.txt" "api_openai*.txt" "api_gemini*.txt" "yandex_key.txt" "yandex_key_id.txt" "yandex_folder.txt" "path_history.json" >nul
set "RC=%errorlevel%"
if %RC% GEQ 8 goto ERR_STAGE
if exist "%STAGE_PROJECT%\install\init_folders.cmd" call "%STAGE_PROJECT%\install\init_folders.cmd" >nul
...
:BANNER
echo ======================================================================
echo   AUDION OFFICE OCR AI - MAKE RELEASE ARCHIVE
echo ======================================================================
echo Root:    %ROOT%
