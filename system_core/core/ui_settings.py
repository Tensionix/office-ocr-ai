...
def load_ui_settings(path: Path) -> UiSettings:
    ...
    return UiSettings(
        ...
        source_path=str(ui_data.get("source_path", "") or ""),
        destination_path=str(ui_data.get("destination_path", "") or ""),
    )
...
def save_ui_settings(path: Path, settings: UiSettings) -> None:
    ...
    text = (
        ...
        f"  source_path: \"{settings.source_path.replace(chr(92), chr(92) + chr(92)).replace(chr(34), chr(92) + chr(34))}\"\n"
        f"  destination_path: \"{settings.destination_path.replace(chr(92), chr(92) + chr(92)).replace(chr(34), chr(92) + chr(34))}\"\n"
    )
