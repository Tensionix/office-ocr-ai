...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--smoke", action="store_true", help="Validate window defaults without starting a server.")
...
def main() -> int:
    ...
    if args.smoke:
        print(
            f"OK native window: {ROOT} | {args.host}:{args.port} | "
            f"{UI_DEFAULTS['width']}x{UI_DEFAULTS['height']}"
        )
        return 0
